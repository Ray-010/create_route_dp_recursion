...
class PathCreation:
    ...
    def check_passable(self, route, x) -> bool:
        passable = False
    ...
    def dp_backward(self, dp1):
        ...
        for i in range(self.operation_num-1, -1, -1):
            if dp1[i][self.goal_num]:
                operation = i
                break
        if operation == -1: return None

        dp2[operation][self.goal_num] = [-1] # 番兵
        for i in self.adjacency_list[self.goal_num]:
            dp2[operation-1][i].append(self.goal_num)
    ...
    def find_route_by_bitdfs(self, dp):
